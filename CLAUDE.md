...
- `OTEL_EXPORTER_OTLP_METRICS_TEMPORALITY_PREFERENCE` is set to `delta` in `setup_tracing()` via `os.environ.setdefault()` before any exporter is created. Dynatrace rejects `CUMULATIVE` monotonic sums (`UNSUPPORTED_METRIC_TYPE_MONOTONIC_CUMULATIVE_SUM`), and the OTel HTTP exporter defaults to `CUMULATIVE`. The env var approach is used rather than the `preferred_temporality` constructor argument because the SDK validates that dict's keys against its own internal instrument class references, which differ from the public API classes and cause a runtime error.
