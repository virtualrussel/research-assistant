...
- The `OTLPMetricExporter` is configured with `preferred_temporality` set to `DELTA` for all sum and histogram instruments. Dynatrace rejects `CUMULATIVE` monotonic sums (`UNSUPPORTED_METRIC_TYPE_MONOTONIC_CUMULATIVE_SUM`), and the OTel Python HTTP exporter defaults to `CUMULATIVE`. Gauges use `CUMULATIVE` (they report instantaneous values, not increments).
