...
- `Traceloop.init()` is called with `exporter=` (not `api_endpoint=`). This prevents Traceloop from creating a second OTLP trace exporter internally and avoids it configuring metrics at its own default endpoint.
